...
async function fireMoveAction ( destFolder, messageList ){
    ...
    await messenger.storage.local.set( { history: historyArray } ).then( {} );
    if ( action == ACTION_MOVE || action == ACTION_MOVEJUMP) {
	browser.runtime.sendMessage( { 'type': 'move', 'destFolder': destFolder, 'messages': messageList.messages })
    } 
    if ( action == ACTION_JUMP || action == ACTION_MOVEJUMP) {
	browser.runtime.sendMessage( { 'type': 'show', 'destFolder': destFolder })
    }
